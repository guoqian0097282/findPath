...
struct CenterlinePoint {
    ...
    CenterlinePoint() : curvature(0), heading(0), confidence(1.0), 
...
public:
    ...
    std::vector<CenterlinePoint> extract(const cv::Mat& occupancy_grid) {
        ...
        std::vector<cv::Point2d> left_boundary, right_boundary;
        ...
        if (centerline.size() > 20) {
            centerline = SmoothingFilter::movingAverage(centerline, 20);
...
public:
    TemporalProcessor(int max_history = 15) : max_history_(max_history) {}
