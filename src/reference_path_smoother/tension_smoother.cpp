...
bool TensionSmoother::osqpSmooth(const std::vector<double> &x_list,
    ...
    if(x_list.size() != y_list.size() || y_list.size()!=angle_list.size() || angle_list.size()!=s_list.size())
    {
        std::cout<<"osqpSmooth x_list y_list angle_list is not equation"<<std::endl;
    }
