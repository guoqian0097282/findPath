...
bool ReferencePathSmoother::graphSearchDp(std::shared_ptr<PathOptimizationNS::ReferencePath> reference) {
    ...
    if (fabs(vehicle_local.y) > FLAGS_search_lateral_range) {
        std::cout << "Vehicle far from ref, quit graph search.";
        return false;
    }
...
bool ReferencePathSmoother::graphSearch(std::shared_ptr<PathOptimizationNS::ReferencePath> reference) {
    ...
    while (true) {
        if (open_set_.empty()) {
            std::cout << "Lattice search failed!";
            break;
        }
...
void ReferencePathSmoother::path2st() {
    ...
    std::cout<<"path2st length:"<<length;    
    s_list_.emplace_back(0);
    std::cout<<" x_list_size:"<<x_list_.size();

    for (size_t i = 1; i != x_list_.size(); ++i) {
        double dis = sqrt(pow(x_list_[i] - x_list_[i - 1], 2) + pow(y_list_[i] - y_list_[i - 1], 2));
        s_list_.emplace_back(s_list_.back() + dis);
    }
    std::cout<<" s_list_:"<<s_list_.size();
}
...
bool ReferencePathSmoother::postSmooth(std::shared_ptr<PathOptimizationNS::ReferencePath> reference_path) {
    ...
    if (point_num < 4) {
        std::cout << "Ref is short: " << point_num << ", quit POST SMOOTHING.";
        return false;
    }
    ...
    if (!solver.solve()) {
        std::cout << "Post smoothing QP failed!";
        return false;
    }
