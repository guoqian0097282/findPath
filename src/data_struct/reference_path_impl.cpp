...
void ReferencePathImpl::updateBoundsOnInputStates(const Map &map, std::vector<SlState> &input_sl_states) {
    ...
    if(input_sl_states.size() > reference_states_.size())
    {
        std::cout<<"updateBoundsOnInputStates input_sl_states.size > reference_states_.size"<<std::endl;
    }
    if (reference_states_.empty()) {
        std::cout << "Empty reference, updateBounds fail!";
        return;
    }
    bounds_.clear();
    ...
    for (size_t i = 0; i < input_sl_states.size(); ++i) {
        ...
        if (isEqual(front_bound[0], front_bound[1]) || isEqual(rear_bound[0], rear_bound[1])) {
            std::cout << "Path is blocked at s: " << ref_state.s;
...
bool ReferencePathImpl::buildReferenceFromSpline(double delta_s_smaller, double delta_s_larger) {
    ...
    {
        std::cout<<" buildReferenceFromSpline delta_s_smaller > delta_s_larger"<<std::endl;
    }
