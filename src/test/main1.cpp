...
#define LH_PATHPLANNING_ALG_VERSION ("LH-PAHTPLAN-V1.51") // pathplanning算法版本号
    ...
    void referenceTurnPoint(int endIndex, int index, vector<Vec6d> pathPoints, double thX, double thY)
    {
        ...
        {
            double xs = pathPoints[index + i][1] - height/2*resolution + 0.8;
            double ys = -pathPoints[index + i][0] + width/2*resolution;
...
int main() 
{
    ...
    int LocalPathPlanning(const cv::Mat& image, std::vector<T_Obs> Obs, const std::vector<CenterlinePoint>& centerPoints,T_CarState tCarState, const char *tempFile, bool needPath, double cur_speed)
    {
        ...
        for(int i=0;i<centerPoints.size();i++)
        {
            ...
            point[1] = abs(centerPoints[i].position.y-10);
        ...
        if (numSize > 0)
        {
            ...
            if ((pow((tCarState.dX - LocalPath[numSize - 1].x), 2) + pow((tCarState.dY - LocalPath[numSize - 1].y), 2)) < 16)
            {
                shortPath = true;
            }
        }
        ...
        if ((isFirst || needPath || travelDistance > 64 || shortPath))
        {
            ...
            while (centerPoint.size() == 0)
            {
                ...
                for (int i = 0; i < contours.size(); i++)
                {
                    ...
                    if (cv::contourArea(contours[i]) > 5000)
                    {
                        ...
                        if(point.x < 1360 && point.x > 560 && point.y < 400)
            ...
            {
                double distance = sqrt(pow((pathPoints[0][0] - pathPoints[i][0]), 2) + pow((pathPoints[0][1] - pathPoints[i][1]), 2));
            ...
            pixelpathPoint.y = height - endpathPoint.x / resolution;
            double minDis = 10000;
            ...
            start_state.x = (-height / 2) * resolution + 2;
