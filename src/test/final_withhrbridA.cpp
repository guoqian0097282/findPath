...
struct State {
    ...
    double g;       // 从起点到当前状态的实际代价
    double f;       // 总估计代价(f = g + h)
...
namespace std {
    ...
    struct hash<State> {
        size_t operator()(const State& s) const {
            return hash<int>()(s.x) ^ hash<int>()(s.y) ^ hash<double>()(s.theta);
        }
    };
}
...
// 考虑方向的启发式
double hybridHeuristic(const State& current, const State& goal, double max_curvature) {
    double dx = goal.x - current.x;
    double dy = goal.y - current.y;
    double distance = std::sqrt(dx * dx + dy * dy);
    
    // 考虑方向差异
    double angle_diff = std::abs(std::atan2(dy, dx) - current.theta);
    angle_diff = std::min(angle_diff, 2 * M_PI - angle_diff);
    
    // 考虑最小转弯半径
    double turning_cost = angle_diff * max_curvature;
    
    return distance + turning_cost;
}

// 生成后继状态
std::vector<State> getSuccessors(const State& current, const std::vector<std::vector<int>>& grid, 
                                double step_size, double max_steering_angle, int num_steering_angles) {
    std::vector<State> successors;
    
    // 离散转向角度
    for (int i = -num_steering_angles; i <= num_steering_angles; ++i) {
        double steering_angle = max_steering_angle * i / num_steering_angles;
        
        // 自行车模型运动
        double new_theta = current.theta + std::tan(steering_angle) * step_size;
        int new_x = current.x + std::cos(new_theta) * step_size;
        int new_y = current.y + std::sin(new_theta) * step_size;
...
std::vector<State> hybridAStar(const State& start, const State& goal, 
                              ...
                              int num_steering_angles = 3) {
    // 优先队列(开放列表)
    std::priority_queue<State, std::vector<State>, std::greater<State>> open_list;
    
    // 存储所有节点
    std::unordered_map<State, NodeMap> all_nodes;
    
    // 初始化起点
    State start_state = start;
    start_state.g = 0;
    start_state.f = hybridHeuristic(start_state, goal, 1.0/max_steering_angle);
    
    open_list.push(start_state);
    all_nodes[start_state] = {start_state, start_state, false};
    ...
    while (!open_list.empty()) {
        ...
        // 标记为已关闭
        all_nodes[current].closed = true;
        
        // 检查是否到达目标
        if (euclideanHeuristic(current.x, current.y, goal.x, goal.y) < 1.0 && 
            std::abs(current.theta - goal.theta) < M_PI/4) {
            // 回溯路径
        ...
        // 生成后继状态
        auto successors = getSuccessors(current, grid, step_size, max_steering_angle, num_steering_angles);
        
        for (auto& next : successors) {
            // 计算启发式值
            next.f = next.g + hybridHeuristic(next, goal, 1.0/max_steering_angle);
            
            // 检查是否已经探索过该状态
            if (all_nodes.find(next) == all_nodes.end()) {
                // 新状态，加入开放列表
                open_list.push(next);
                all_nodes[next] = {next, current, false};
            } else if (next.g < all_nodes[next].state.g) {
                // 找到更优路径，更新状态
                all_nodes[next].state = next;
                all_nodes[next].parent = current;
                
                // 如果不在开放列表中，重新加入
                if (all_nodes[next].closed) {
                    open_list.push(next);
                    all_nodes[next].closed = false;
                }
            }
        }
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < all_file_names.size(); i++) // all_file_names.size(); i++)
    {
        // i = 87;
        ...
        if (load_ret)
        {
            ...
            else
            {
                ...
                ifstream inFile("/home/gq/guoqian/data/save_slamGQ_1.txt", ios::in | ios::binary);
            ...
            for (int i = 0; i < j3Obs.size(); i++)
            {
                ...
                if (is_oblique)
                {
                    j3Obs[i].obs_width = j3Obs[i].obs_length;
                }
            ...
            for (int j = 0; j < 1; j++) // initPose.size()
            {
                j = 2;
                car_state.x = initPose[j][0];       //  Y: T:-1.643736
                car_state.y = initPose[j][1];       // currentPose[j][1];              //-78.2019;//0.000;//-7.16728;
                car_state.heading = initPose[j][2]; // currentPose[j][2] + 1.5706; // 4.70266;//-0.000796446;//-1.31224;
                ...
                State start = {startY - 5, startX,0};
                State goal = {goalY, goalX,car_state.heading};

                std::vector<State> path = hybridAStar(start, goal, gdMap);
                double delta = 0;
