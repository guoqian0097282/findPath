...
double vehicle_length = 4.93;
double vehicle_width = 2.11;
namespace
{
...
{
  planning_time_ = 4; //planning_time;
  planning_distance_ = planning_distance;
  discretizedPath_ = discretizedPath;
  adc_path_init_s_ = discretizedPath_.front().s;
...
bool STObstaclesProcessor::MapObstaclesToSTBoundaries(std::vector<Obstacle> Obstacles)
{
  // Sanity checks.
  ...
  for (int i = 0; i < Obstacles.size(); i++)
  {
    ...
    {
      std::cout<<lower_points[i].s()<<","<<lower_points[i].t()<<std::endl;
      std::cout<<upper_points[i].s()<<","<<upper_points[i].t()<<std::endl;
    }
    auto boundary =
    ...
    std::cout << "Obstacle " << obs_ptr->Id()
              << " has an alternative st-boundary with "
              << lower_points.size() + upper_points.size() << " points."<<std::endl;;
    ...
    else
    {
      ...
      std::cout << "Adding " << obs_ptr->Id() << " into the ST-graph."<<std::endl;;
    }
  }
  ...
  if (std::get<0>(closest_stop_obstacle) != "NULL")
  {
    ...
    std::cout << "Closest obstacle ID = " << closest_stop_obs_id<<std::endl;
    ...
    if (!closest_stop_obs_ptr->IsVirtual())
    {
      for (const auto &clear_zone : candidate_clear_zones_)
      {
            ...
            closest_stop_obs_boundary.min_s() <= clear_zone_boundary.max_s())
        {
          ...
          std::cout << "Clear zone " << closest_stop_obs_id << " is closer."<<std::endl;
          break;
        }
      }
    }
    ...
    std::cout << "Adding " << closest_stop_obs_ptr->Id() << " into the ST-graph."<<std::endl;
    std::cout << "min_s = " << closest_stop_obs_boundary.min_s()<<std::endl;
  }
    ...
    double *const obs_caution_end_t)
{
  ...
  *is_caution_obstacle = false;
  const auto &adc_path_points = discretizedPath_;
  const auto &obs_trajectory = obstacle.Trajectory();

  if (obs_trajectory.trajectory_point.size() == 0)
  {
    // Processing a static obstacle.
    // Sanity checks.
    if (!obstacle.IsStatic())
    {
      std::cout << "Non-static obstacle[" << obstacle.Id()
  ...
  else
  {
    ...
    for (const auto &obs_traj_pt : obs_trajectory.trajectory_point)
    {
      ...
      if (GetOverlappingS(adc_path_points, obs_box, kADCSafetyLBuffer,
                          &overlapping_s))
      {
        std::cout << "Obstacle instance is overlapping with ADC path."<<std::endl;
    ...
    std::pair<double, double> *const overlapping_s)
{
  ...
  int pt_before_idx = GetSBoundingPathPointIndex(
      adc_path_points, obstacle_instance, front_edge_to_center,
      true, 0, static_cast<int>(adc_path_points.size()) - 2);
  // std::cout << "The index before is " << pt_before_idx<<std::endl;
  int pt_after_idx = GetSBoundingPathPointIndex(
      adc_path_points, obstacle_instance, back_edge_to_center,
      false, 0, static_cast<int>(adc_path_points.size()) - 2);
  // std::cout << "The index after is " << pt_after_idx<<std::endl;
  ...
  {
    // std::cout << "At ADC path index = " << i << " :"<<std::endl;
                                     ...
                                     adc_l_buffer))
    {
      ...
      std::cout << "There is overlapping."<<std::endl;
      break;
    }
  }
  if (!has_overlapping)
  ...
  {
    // std::cout << "At ADC path index = " << i << " :";
    if (IsADCOverlappingWithObstacle(adc_path_points[i], obstacle_instance,
                                     adc_l_buffer))
    {
      overlapping_s->second = adc_path_points[i + 1].s;
      std::cout << "There is overlapping.";
      break;
    }
  }
  return true;
    ...
    const double l_buffer) const
{
  ...
  Vec2d ego_center_map_frame((front_edge_to_center -back_edge_to_center) *0.5,(left_edge_to_center -right_edge_to_center) *0.5);
  ...
  Box2d adc_box(ego_center_map_frame, adc_path_point.theta,
                vehicle_length, vehicle_width + l_buffer * 2);
