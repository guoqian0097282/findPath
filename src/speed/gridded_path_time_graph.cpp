                  ...
                  st_graph_data_.st_drivable_boundary(), init_point_)
{
  ...
  max_deceleration_ = gridded_path_time_graph_config_.max_deceleration;
}
  ...
  for (const auto &boundary : st_graph_data_.st_boundaries())
  {
         ...
         std::fabs(boundary->min_s()) < kBounadryEpsilon))
    {
      ...
      for (uint32_t i = 0; i < dimension_t_; ++i, t += unit_t_)
      {
        ...
        speedPoint.s = 0;
        speedPoint.t = t;
        speed_profile.push_back(speedPoint);
      }
...
bool GriddedPathTimeGraph::RetrieveSpeedProfile(SpeedData *const speed_data)
{
  ...
  if (speed_profile[speed_profile.size() - 2].v != 0)
  {
    speed_profile[speed_profile.size() - 2].v = speed_profile[speed_profile.size() - 3].v;
  }

  for (int i = 0; i + 1 < speed_profile.size(); i++)
  {
    std::cout << "Time: " << speed_profile[i].t << "S: " << speed_profile[i].s << "V: " << speed_profile[i].v << std::endl;
  }
