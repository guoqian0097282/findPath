...
std::string STBoundary::TypeName(BoundaryType type) {
  ...
  std::cout << "Unknown boundary type " << static_cast<int>(type)
        << ", treated as UNKNOWN";
  return "UNKNOWN";
}
...
bool STBoundary::GetUnblockSRange(const double curr_time, double* s_upper,
  ...
  *s_upper = 200;//FLAGS_speed_lon_decision_horizon;
  *s_lower = 0.0;
  ...
  // } else {
  //   std::cout << "boundary_type is not supported. boundary_type: "
...
bool STBoundary::IsPointInBoundary(const STPoint& st_point) const {
  ...
  const double check_upper = CrossProd(
      st_point, upper_points_[left], upper_points_[right]);
  const double check_lower = CrossProd(
...
STBoundary STBoundary::ExpandByT(const double t) const {
  if (lower_points_.empty()) {
    std::cout << "The current st_boundary has NO points.";
    return STBoundary();
  }
...
bool STBoundary::IsValid(
  ...
  if (point_pairs.size() < 2) {
    std::cout << "point_pairs.size() must >= 2, but current point_pairs.size() = "
           << point_pairs.size();
    return false;
  }
  ...
  for (size_t i = 0; i < point_pairs.size(); ++i) {
    ...
    if (curr_upper.s() < curr_lower.s()) {
      std::cout << "ST-boundary's upper-s must >= lower-s";
      return false;
    }
    if (std::fabs(curr_lower.t() - curr_upper.t()) > kStBoundaryEpsilon) {
      std::cout << "Points in every ST-point pair should be at the same time.";
      return false;
    }
    if (i + 1 != point_pairs.size()) {
      ...
      if (std::fmax(curr_lower.t(), curr_upper.t()) + kMinDeltaT >=
        ...
        std::cout << "Latter points should have larger t: ";
        return false;
      }
    }
  }
...
bool STBoundary::IsPointNear(const LineSegment2d& seg,
